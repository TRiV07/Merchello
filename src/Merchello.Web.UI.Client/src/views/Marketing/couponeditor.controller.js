...
angular.module('merchello').controller('Merchello.Backoffice.OfferEditController',
    ['$scope', '$routeParams', '$location', '$filter', 'merchDateHelper', 'assetsService', 'dialogService', 'eventsService', 'notificationsService', 'localizationService', 'settingsResource', 'marketingResource', 'merchelloTabsFactory',
        ...
        function ($scope, $routeParams, $location, $filter, dateHelper, assetsService, dialogService, eventsService, notificationsService, localizationService, settingsResource, marketingResource, merchelloTabsFactory,
            dialogDataFactory, settingDisplayBuilder, offerProviderDisplayBuilder, offerSettingsDisplayBuilder, offerComponentDefinitionDisplayBuilder) {
            ...
            function loadOfferProviders() {
                var providersPromise = marketingResource.getOfferProviders($routeParams.storeId);
            ...
            function loadOffer(key) {
                ...
                if (key === 'create' || key === '' || key === undefined) {
                    ...
                    $scope.offerSettings.storeId = $routeParams.storeId;
                    createTabs(key);
            ...
            function saveOffer() {
                ...
                if ($scope.offerForm.$valid) {
                    ...
                    offerPromise.then(function (settings) {
                        ...
                        if (isNew) {
                            $location.url($scope.offerProvider.editorUrl(settings.key + '/store/' + $routeParams.storeId), true);
                        } else {
            ...
            function processDeleteOfferConfirm(dialogData) {
                ...
                promiseDelete.then(function () {
                    $location.url('/merchello/merchello/offerslist/manage' + '/store/' + $routeParams.storeId, true);
                }, function (reason) {
