namespace Merchello.Web.Editors.Reports
{
    ...
    internal static class ReportSqlHelper
    {
        ...
        public static class SalesByItem
        {
            ...
            /// The <see cref="Sql"/>.
            /// </returns>
            public static Sql GetSkuSaleCountSql(DateTime startDate, DateTime endDate, Guid typeFieldKey, int storeId, int count = 10)
            {
                var sql = @"SELECT TOP(@top) Q1.Sku,
                    ...
                    FROM (
		                    ...
		                    INNER JOIN (
				                    ...
				                    AND	MI.invoiceDate BETWEEN @start AND @end AND MI.storeId = @storeId
                ...
                return new Sql(sql, new { @top = count, @start = startDate, @end = endDate, @tfKey = typeFieldKey, @storeId = storeId });
            }
        }
